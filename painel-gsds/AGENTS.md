...
- Observatório de startups: 5A–5D.1 autorizadas (Brazil/Global indirect + 28 players do mapeamento direto global). Não importar CSV/MD bruto de scouting/handoff, Classe C, unresolved, filas CB, bot, `ResearchRun`, candidatos, staging ou `ChangeSet` sem nova autorização. Não colapsar `organization_status` / `current_gsd_activity` / `asset_role`. Não fabricar `publicSources` a partir de website institucional. Não renderizar `HISTORICAL_OWNER` nem ownership histórico para Kriya; Maze/Valerion usam `historical-owner`.
