...
4. **Fronteira de publicação:** o app público lê **somente** `src/data/startups/published/current.json` → snapshot imutável em `published/snapshots/`. O seletor não duplica o corpo do snapshot. O loader descobre automaticamente, em build time, todos os JSON publicados (`import.meta.glob`) e os indexa pelo `id` interno — sem registry manual. Fixtures ficam em `tests/fixtures/startups/` e fora do glob de produção.
