...
diagnostics.clinical1440 = {
  ...
  notice: await computedProbe(page, '.research-header__notice'),
  lockupCount: await page.locator('img.brand__lockup').count(),
  brandMarkCount: await page.locator('.brand__mark').count(),
  medicalNoticeCount: await page.locator('.research-header__notice').count(),
  genericMedicalCount:
...
for (const width of [1152, 1024, 768]) {
  ...
  await page.goto(url('analises/bases-clinicas/'), { waitUntil: 'networkidle' });
  diagnostics[`clinical${width}`] = {
    overflow: await pageOverflow(page),
    header: await headerChrome(page),
    lockupCount: await page.locator('img.brand__lockup').count(),
  };
  await fullShot(page, `clinical-${width}.png`);
}

async function captureClinicalMenu(width) {
  ...
  diagnostics[`clinicalMenu${width}`] = {
    ...menu,
    alpha: parseAlpha(menu.backgroundColor),
  };
  await page.screenshot({
    path: path.join(outDir, `clinical-menu-${width}.png`),
    fullPage: false,
  });
  await page.keyboard.press('Escape');
}

await captureClinicalMenu(375);
await captureClinicalMenu(320);

await page.setViewportSize({ width: 320, height: 640 });
await page.goto(url('analises/bases-clinicas/'), { waitUntil: 'networkidle' });
await page.locator('.table-wrap').scrollIntoViewIfNeeded();
const table = page.locator('.table-wrap');
diagnostics.clinicalTable320 = {
  overflow: await pageOverflow(page),
...
await table.screenshot({ path: path.join(outDir, 'clinical-table-320.png') });
...
await browser.close();
console.log('6D.4.1 Clínica screenshots written to', outDir);
